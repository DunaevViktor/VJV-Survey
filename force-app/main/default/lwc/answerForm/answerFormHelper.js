...
const compareValues = (answerValue, operator, validationValue) => {
    ...
    switch (operator) {
      case "IS NULL":
        isConditionMet = (answerValue == null) === validationValue;
        //isConditionMet = answerValue === validationValue; // null
        break;
      case "EQUALS":
    ...
    const dependentQuestion = questions.findIndex(
     ...
     questions[dependentQuestion].IsVisible__c = false;
      if (questions[dependentQuestion].Validations1__r != null) {
        questions[dependentQuestion].Validations1__r.forEach(
          (nextValidation) => {
            hideAnswerChain(questions, nextValidation); //clear value
          }
        );
      }
}
