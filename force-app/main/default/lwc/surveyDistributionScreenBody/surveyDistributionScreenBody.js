...
export default class SurveyDistributionScreenBody extends LightningElement {
    ...
    get groupOptions() {
        return this.groups.data.map(
            (group) => {return {label: group.Name, value: group.Id}});
    }
    ...
    handleAddEmailReceiver(inputForm, receiver){
        ...
        let validityMessage = "";
        console.log("shit");
        if(this.data.find((tempReceiver, index) => {if(receiver.nameOrEmail.localeCompare(tempReceiver.nameOrEmail) === 0) return true;})){
    ...
    handleAddGroupReceiver(combobox, receiver){
        ...
        console.log("shit2");
        receiver.type = 'Group';
        receiver.nameOrEmail = this.groups.data.find((group, index) => {if(this.groupId == group.Id) return true;}).Name;
    ...
    get surveyOptions(){
        return this.surveys.map(
            (survey) => {return {label: survey.Name, value: survey.Id}});
    }
